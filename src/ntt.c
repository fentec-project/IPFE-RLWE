...
void poly_mul_ntt(uint32_t a[MIFE_N], uint32_t b[MIFE_N],uint32_t c[MIFE_N], uint32_t sel){
	...
	uint32_t a_t[MIFE_N], b_t[MIFE_N];
	...
	for(m=1; m<MIFE_N; m=2*m){
		...
		for(i=0;i<m;i++){
			...
			for(j=j1; j<=j2; j++){
				...
				//printf("Accessing array %ld\n", j+t);
				U=a[j];
				//V=reduce( (uint64_t)(a[j+t]*S), MIFE_MOD_Q_I[sel] );
				V=mul_mod_ntt( a[j+t], S, sel );
				//a[j]=reduce ( (uint64_t)(U+V), MIFE_MOD_Q_I[sel] );
				a[j]=add_mod_ntt(U, V, sel);
				//a[j+t]=reduce( (uint64_t)(U-V), MIFE_MOD_Q_I[sel] );
...
void GS_reverse(uint32_t a[MIFE_N], uint32_t sel){
	...
	for(m=MIFE_N; m>1; m=m/2) {
		...
		for(i=0;i<h;i++){
			...
			for(j=j1;j<=j2;j++){
				...
				U=a[j];
				V=a[j+t];
				//a[j]=reduce( U+V, MIFE_MOD_Q_I[sel] );
				a[j]=add_mod_ntt(U, V, sel);
				//a[j+t]=reduce( (uint64_t)((U-V)*S), MIFE_MOD_Q_I[sel] );
				a[j+t]=sub_mod_ntt(U, V, sel);
	...
	for(i=0;i<MIFE_N;i++){
		//a[i]=reduce( ((uint64_t)a[i]*(uint64_t)MIFE_NTT_NINV[sel]), MIFE_MOD_Q_I[sel]);	
		a[i]=mul_mod_ntt(a[i], MIFE_NTT_NINV[sel], sel);
	}
}
