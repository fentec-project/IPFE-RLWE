...
void
rlwe_mife_encrypt_vec
(uint32_t m[MIFE_L][MIFE_N], const uint32_t mpk[MIFE_L+1][MIFE_NMODULI][MIFE_N], uint32_t c[MIFE_L+1][MIFE_NMODULI][MIFE_N])
{
	int i, j, k;
	uint64_t r[MIFE_N], f[MIFE_N];
	uint32_t r_crt[MIFE_NMODULI][MIFE_N], f_crt[MIFE_NMODULI][MIFE_N];
	...
	for (i = 0; i < MIFE_L; ++i) {
		...
		for (j = 0; j < MIFE_NMODULI; ++j) {
			for (k = 0; k < MIFE_N; ++k) {
				...
				m_crt[i][j][k] = mod_red(mxm, MIFE_MOD_Q_I[j]);
			}
		}
	}
	...
	for (i = 0; i < MIFE_L; ++i) {
		...
		for (j = 0; j < MIFE_NMODULI; ++j) {
			poly_mul_mod(mpk[i][j], r_crt[j], c[i][j], MIFE_MOD_Q_I[j]);
			add_mod(c[i][j], f_crt[j], c[i][j], MIFE_MOD_Q_I[j]);
			add_mod(c[i][j], m_crt[i][j], c[i][j], MIFE_MOD_Q_I[j]);
			//for (int k = 0; k < MIFE_N; ++k) {
			//	c[i][j][k] = mod_red((c[i][j][k] + m_crt[j][i]), MIFE_MOD_Q_I[j]);
			//}
		}
...
void
rlwe_mife_keygen_vec
(const uint32_t y[MIFE_L][MIFE_N], const uint32_t msk[MIFE_L][MIFE_NMODULI][MIFE_N], uint32_t sk_y[MIFE_NMODULI][MIFE_N])
{
	int i, j, k;
	uint64_t mac;

	uint32_t y_crt[MIFE_L][MIFE_NMODULI][MIFE_N];
	...
	for (i = 0; i < MIFE_L; ++i) {
		crt_convert(y[i], y_crt[i]);
		for (j = 0; j < MIFE_NMODULI; ++j) {
			poly_mul_mac_mod(y_crt[i][j], msk[i][j], sk_y[j], MIFE_MOD_Q_I[j]);
			//for (k = 0; k < MIFE_N; ++k) {
			//	mac = (uint64_t)y_crt[j][i]*msk[i][j][k];
			//	mac = mac + sk_y[j][k];
			//	sk_y[j][k] = mod_red(mac, MIFE_MOD_Q_I[j]);
...
void
rlwe_mife_decrypt_vec
(const uint32_t c[MIFE_L+1][MIFE_NMODULI][MIFE_N], const uint32_t y[MIFE_L][MIFE_N], const uint32_t sk_y[MIFE_NMODULI][MIFE_N], uint64_t dy[MIFE_N])
{
	int i, j, k;
	uint64_t mac;
	...
	for (i = 0; i < MIFE_L; ++i) {
		crt_convert(y[i], y_crt[i]);
		for (j = 0; j < MIFE_NMODULI; ++j) {
			poly_mul_mac_mod(y_crt[i][j], c[i][j], d_y[j], MIFE_MOD_Q_I[j]);
			//for (k = 0; k < MIFE_N; ++k) {
			//	mac = (uint64_t)y_crt[j][i]*c[i][j][k];
			//	mac = mac + d_y[j][k];
			//	d_y[j][k] = mod_red(mac, MIFE_MOD_Q_I[j]);
